...
plot_threshold_maps = function(data_dir, fig_dir,nc_out_dir,shp_file,overwrite_plot =T, day_thresholds = 5){
  ...
  plot_fn = function(data.plot){
    p = ggplot2::ggplot() +
      ...
      ggplot2::geom_sf(data = coast, color = "black", linewidth = 0.5) +
      ...
      ggplot2::scale_fill_viridis_c(na.value = "transparent", name = "Raster Value") +
      ...
      ggplot2::labs(
        title = "Raster with Overlay and Coastline",
        x = "Longitude",
        y = "Latitude"
      )
    return(p)
  ...
  fig.name.mean = paste0(fig_dir,file.df$id[i],'_mean.png')
  fig.name.sd = paste0(fig_dir,file.df$id[i],'_sd.png')
  ...
  #write netcdf out
  nc.name.mean = paste0(nc_out_dir,file.df$id[i],'_mean.nc')
  nc.name.sd = paste0(nc_out_dir,file.df$id[i],'_sd.nc')
  ...
  for(thresh in day_thresholds){
    ...
    fig.name.prob = paste0(fig_dir,file.df$id[i],'_prob_above_',thresh,'days.png')
    nc.name.prob = paste0(nc_out_dir,file.df$id[i],'_prob_above_',thresh,'days.nc')
