...
for(i in 1:length(years)){
  ...
  for(j in 1:nrow(SAM.combs)){
    ...
    writeCDF(SAM.mask,paste0(out.dir,'SAMS/scallop/GLORYS_BT_',SAM.combs$max.vals[i],'_mask_',years[i],'.nc'))
...
write.csv(SAMS.area,here::here('data','habitat_area','Annual_SAMS_thermal_habitat_area.csv'),row.names = F)
