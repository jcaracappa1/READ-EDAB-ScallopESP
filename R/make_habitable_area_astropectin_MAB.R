...
for(i in 1:length(years)){
  ...
  for(j in 1:length(min.vals)){
    ...
    writeCDF(SAM.mask,paste0(out.dir,'/SAMS/astropectin/shelf/GLORYS_BT_SAMS_mask_',min.vals[j],'_',years[i],'.nc'),overwrite =T)
...
write.csv(SAMS.area,here::here('data','habitat_area','thermal_habitat_area_astropectin_SAMS_combined.csv'),row.names = F)
